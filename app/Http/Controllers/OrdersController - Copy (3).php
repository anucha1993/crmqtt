...
class OrdersController extends Controller
{
                            ...
                            <div class="dropdown">
                                ...
                                if($data->status == 1)
                                {
                                    $html .= '<a class="dropdown-item" href="'.url('orders/delivery/list/'.$data->id).'">รายการบิลย่อย</a>
                                            <a class="dropdown-item" href="'.url('orders/payment/'.$data->id).'">ประวัติการชำระ</a>';
                                }
                        $html .= '</div>
                            </div>
                        </td>';
    ...
    public function view($id)
    {
        ...
        return view('orders.view',compact('quotation','breadcrumb','location_contact_person_name','location_contact_person_phone_no','orders','customer','datas','datas_sub_total','delivery_location','i','pocketmoney','historypayment','sumpayment','orders_pera','orders_pera_sql','location_name'));
    }
    ...
    public function payment($id)
    {
        ...
        ->where('payment_history.order_id',$id)->orderby('created_at','desc')->get();
        $breadcrumb = [['url'=>'orders','title'=>'รายการบิลหลัก'],['url'=>'orders/view/'.$orders->id,'title'=>'รายการบิลหลัก '.$orders->order_number],['url'=>'','title'=>'ประวัติการชำระ']];
        $i = 1;
