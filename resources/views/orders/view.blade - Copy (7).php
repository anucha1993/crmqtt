	...
	function open_to_select_product(i_pera)
	 {
		...
		window.open("/orders/assign_pocket_money?i_pera="+i_pera,"_blank","height=100,width=400,status=yes,toolbar=no,menubar=no,location=no")
	 }
</script>
@section('content')
<div class="row">
          ...
          <div class="card-body">
            <div class="form-inline">
                ...
                <div class="text-right col-sm-6">
                    ...
                    <button type="button" class="btn btn-print "  onclick="printBilling_pera('bill');"><i class="fa fa-print" aria-hidden="true"></i> Print บิล (ใบเสร็จ)</button>
					<button type="button" class="btn btn-print "  onclick="printBilling_pera('order');"><i class="fa fa-print" aria-hidden="true"></i> Print บิล (บิลหลัก)</button>
					<div class="btn-group" role="group">
						<button id="btnGroupDrop1" type="button" class="btn btn-secondary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
						  ระบุประเภทการชำระเงิน
						</button>
						<div class="dropdown-menu" aria-labelledby="btnGroupDrop1">
						  <a class="dropdown-item" href="javascript:update_payment_method_type_code(1, '{{$orders->id}}');"> One Time Payment, ชำระครั้งเดียวต่อบิลหลัก </a>
						  <a class="dropdown-item" href="javascript:update_payment_method_type_code(2, '{{$orders->id}}');"> DED, ชำระต่อครั้งของการจัดส่ง </a>
						  <a class="dropdown-item" href="javascript:open_to_select_product({{$orders->id}});"> Pocket Money </a>
						  <a class="dropdown-item" href="javascript:update_payment_method_type_code(2, '{{$orders->id}}');"> Credit </a>
						</div>
				    </div>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <p class="col-sm-10 text-right warning-text">
                    <input type="checkbox" disabled {{($orders->on_vat == 1 || $quotation->qo_on_vat == "Y") ? 'checked' : ''}}  value="1" class="form-control form-control-sm col-sm-1">
                    <label >ภาษีมูลค่าเพิ่ม 7%</label>
                </p>
                <p class="col-sm-2 text-right warning-text">{{ ($orders->on_vat == 1  || $quotation->qo_on_vat == "Y") ? number_format($orders->vat,2) : '0.00'}}</p>
                ...
                <p class="col-sm-2 text-right warning-text">{{ ($orders->on_vat == 1  || $quotation->qo_on_vat == "Y") ? number_format($orders->total,2): number_format($orders->price_all,2) }}</p>
                @if($orders->payment_type == 1)
                            ...
                            <div class="card-body">
								<div class="col-sm-12">
									...
									<div class="d-flex flex-row justify-content-center align-items box-note-status">
										...
										<input class="col-sm-6 form-control form-control-sm" type="text" name="current_delivery_location" id="current_delivery_location" value="{{$location_contact_person_name}}" disabled placeholder="" >
									</div>
									<div class="d-flex flex-row justify-content-center align-items box-note-status">
										<p class="col-sm-2 text-right">เบอร์โทรศัพท์ของผู้ติดต่อ :</p> 
										<input class="col-sm-6 form-control form-control-sm" type="text" name="current_delivery_location" id="current_delivery_location" value="{{$location_contact_person_phone_no}}" disabled placeholder="" >
									</div>
								</div>
<script>
<?php
...
function render_more_info() {
	...
	document.getElementById("existing_delivery_location_contact_person_name").value = existing_delivery_location_contact_person_name[strUser];
	document.getElementById("existing_delivery_location_contact_person_phone_no").value = existing_delivery_location_contact_person_phone_no[strUser];
}
		...
		document.getElementById('new_delivery_location_contact_person_phone_no').disabled = true;
								...
								<div class="col-sm-12">
									<div class="d-flex flex-row justify-content-center align-items box-note-status">
										...
										<select onchange="render_more_info()" class="col-sm-6 form-control form-control-sm" type="text" name="existing_delivery_location_id" disabled id="existing_delivery_location_id">
									...
									<div class="d-flex flex-row justify-content-center align-items box-note-status">
										...
										<input class="col-sm-6 form-control form-control-sm" type="text" name="existing_delivery_location_contact_person_name" id="existing_delivery_location_contact_person_name" value="{{$location_contact_person_name}}" disabled placeholder="" >
									</div>
									<div class="d-flex flex-row justify-content-center align-items box-note-status">
										<p class="col-sm-2 text-right">เบอร์โทรศัพท์ของผู้ติดต่อ :</p> 
										<input class="col-sm-6 form-control form-control-sm" type="text" name="existing_delivery_location_contact_person_phone_no" id="existing_delivery_location_contact_person_phone_no" value="{{$location_contact_person_phone_no}}" disabled placeholder="" >
									</div>
								</div>
								...
								<div class="col-sm-12">
									...
									<div class="d-flex flex-row justify-content-center align-items box-note-status">
										<p class="col-sm-2 text-right">ผู้ติดต่อของที่อยู่จัดส่ง :</p> 
										<input class="col-sm-6 form-control form-control-sm" type="text" name="new_delivery_location_contact_person_name" id="new_delivery_location_contact_person_name" value="{{$location_contact_person_name}}" disabled placeholder="" >
									</div>
									<div class="d-flex flex-row justify-content-center align-items box-note-status">
										<p class="col-sm-2 text-right">เบอร์โทรศัพท์ของผู้ติดต่อ :</p> 
										<input class="col-sm-6 form-control form-control-sm" type="text" name="new_delivery_location_contact_person_phone_no" id="new_delivery_location_contact_person_phone_no" value="{{$location_contact_person_phone_no}}" disabled placeholder="" >
									</div>
								</div>
                                <div class="d-flex flex-row justify-content-center align-items box-note-status">
				...
				<div class="row">
												<div class="col-sm-12">
														<label class="col-sm-6 header-title">หมายเหตุ ที่ 1 <input class="form-control form-control-sm" type="text" id=remark_1 name=remark_1  value="{{$orders->remark_1}}"></label>
														<label class="col-sm-6 header-title">หมายเหตุ ที่ 2 <input class="form-control form-control-sm" type="text" id=remark_2 name=remark_2  value="{{$orders->remark_2}}"></label>
														<label class="col-sm-6 header-title">หมายเหตุ ที่ 3 <input class="form-control form-control-sm" type="text" id=remark_3 name=remark_3  value="{{$orders->remark_3}}"></label>
														<label class="col-sm-6 header-title">หมายเหตุ ที่ 4 <input class="form-control form-control-sm" type="text" id=remark_4 name=remark_4  value="{{$orders->remark_4}}"></label>
														<label class="col-sm-6 header-title">หมายเหตุ ที่ 5 <input class="form-control form-control-sm" type="text" id=remark_5 name=remark_5  value="{{$orders->remark_5}}"></label>
												</div>
											</div>
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="d-flex flex-row justify-content-center align-items">
                            ...
                            <button type="submit" id="render_price" name="render_price" value="Yes" class="btn btn-primary btn-fw">ยืนยันทำรายการแบบ => แสดงราคา</button>
							<button type="submit" id="render_price" name="render_price" value="No" class="btn btn-primary btn-fw">ยืนยันทำรายการแบบ => ไม่แสดงราคา</button>
							<?php 
								$disabled = "";
							...
							?>
							<button type="button" id="button_for_add_history" {{$disabled}} onclick="add_payment_history_record(
																0,
																{{$orders->id}},
																{{$orders->price_all}},
																'<?=date("Y/m/d H:i:s");?>',
...
function add_payment_history_record(order_delivery_id, order_id, amount, date_time) {
	$.ajax({
			...
			type: 'get',
			data: {order_delivery_id:order_delivery_id, order_id:order_id, amount:amount, date_time:date_time},
			//cache: false,
			success: function(res){
				//alert(res);
			},
