...
@section('content')
                ...
                <div class="card-body">
                    <div class="form-inline">
                        ...
                        <div class="text-right col-sm-6">
                            ...
                            <a href="{{ url('orders/delivery/add/' . $order->id) }}" class="btn btn-add mb-2" disabled><i
                                    class="fa fa-plus" aria-hidden="true"></i> เพิ่มบิลย่อย</a>
                    ...
                    <div class="form-row">
                        ...
                        <div class="input-group col-sm-3 input-header">
                            ...
                            <div class="col-sm-12">
                                <p class="title_order">
                                    {{ $quotation->customer_type == 'customer' ? (!empty($customer->PocketMoney) ? number_format($customer->PocketMoney->pocket_money, 2) : '0.00') : '0.00' }}
                                </p>
                            </div>
                        </div>
                    </div>
                            ...
                            <tbody>
                                ...
                                @forelse ($datas as $item)
                                    <tr>
                                        ...
                                        @if ($order->payment_type == 3)
                                            <td>
                                                <a href="#" class="btn-link"><i class="fa fa-print"
                                                        aria-hidden="true"></i> ใบวางบิล</a>
                                            </td>
                                        @endif
                                        <td>
                                            {{-- <a href="{{ route('payment.create',$order->id) }}" data-id="{{ $order->id }}"
                                                class="api-payment">แจ้งชำระเงิน </a> | --}}
                                            <a href="{{ url('orders/delivery/view/' . $item->order_delivery_id) }}"
                                                class="btn-link">รายละเอียดบิลย่อย</a>
                                        </td>
                                    </tr>
                                @empty
                                @endforelse
    ...
    {{-- Modal-api Payment --}}
      ...
      <div class="modal fade" id="api-payment" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true"> 
 ...
 <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js"></script>
        ...
        {{-- <script>
            $(".api-payment").click(function(e) {
                e.preventDefault();
                const modal = $("#api-payment");
                modal.find(".modal-content").html("กำลังโหลด...");
                modal
                    .modal("show")
                    .addClass("modal-lg")
                    .find(".modal-content")
