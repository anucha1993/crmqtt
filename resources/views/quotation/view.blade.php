...
<div class="row">
    ...
    <div class="col-lg-12 grid-margin">
        <div class="card">
          <div class="card-body">
			...
			<div class="form-row">
                <div class="input-group col-sm-12 input-header">
                    <label class="col-sm-12 header-title">เบอร์โทรผู้ติดต่อหน้างาน</label>
                    <div class="col-sm-12">
                        <input class="form-control" type="text"  disabled value="{{$datas_quotation_pera->onsite_contact_phone_no}}">
                    </div>
                </div>
            </div>
	...
	<?php $j_pera = 0 ?>
                            ...
                            <tr class="column" id="row_{{$key+1}}" data-id="{{$key+1}}">
                                ...
                                <td>
									...
									<?php 
										...
										$found_word = strpos($data->pera, "เหล็กข้าง");
				...
				<div class="row">
                    <label class="col-sm-10 text-right m-t-15 ">ภาษีมุลค่าเพิ่ม</label>
                    <label class="col-sm-2 text-right m-t-15 money-color" id="vat_pera">0.00</label>
                </div>
				<div class="row">
                    <label class="col-sm-10 text-right m-t-15 ">ยอดสั่งซื้อก่อน Vat</label>
                    <label class="col-sm-2 text-right m-t-15 money-color" id="total_all_before_vat">0.00</label>
                </div>
                <div class="row">
...
<script>
function vat_checkbox_by_pera() {
	if($('input[name="vat_checkbox"]').is(':checked'))
	{
	  // checked
	  total_all = document.getElementById("total_all").textContent;
	  total_all = total_all.replace(",","");
	  total_all_before_vat = document.getElementById("total_all_before_vat").textContent;
	  vat_pera = total_all * (7/100);
	  
	  vat_pera = vat_pera.toFixed(2);
	  total_all_new = (parseFloat(total_all) + parseFloat(vat_pera));
	  total_all_new = price_with_comma(total_all_new.toFixed(2));
	  
	  document.getElementById("vat_pera").textContent = price_with_comma(vat_pera);
	  document.getElementById("total_all").textContent = total_all_new;
	}else
	{
	 // unchecked
	 total_all_before_vat = document.getElementById("total_all_before_vat").textContent;
	 document.getElementById("vat_pera").textContent = 0.00;
	 document.getElementById("total_all").textContent = total_all_before_vat;
	}
}

function price_with_comma(nStr)
{
	nStr += '';
	x = nStr.split('.');
	x1 = x[0];
				...
				<div class="row">
					<div class="col-sm-12">
						<table style="width:100%;" border=0>
							<tr>
								<td width=90% align=right><label class="text-right">หากต้องการภาษีมูลค่าเพิ่ม<span class="red"></span></label>
								</td>
								<?php
									if ($datas_quotation_pera->qo_on_vat == "Y") {
										$checked = "checked";
										echo "<script>";
										echo 'function complete_vat() {';
										echo 'total_all = document.getElementById("total_all").textContent;';
										echo 'total_all = total_all.replace(",","");';
										
										echo 'vat_pera = total_all * (7/100);';
										echo 'vat_pera = vat_pera.toFixed(2);';
										
										echo 'total_all_new = (parseFloat(total_all) + parseFloat(vat_pera));';
										echo 'total_all_new = total_all_new.toFixed(2);';
										  
										echo 'document.getElementById("vat_pera").textContent = price_with_comma(vat_pera);';
										echo 'document.getElementById("total_all").textContent = price_with_comma(total_all_new);';
										echo '}';
										echo 'complete_vat();';
										echo "</script>";
									} else {
								...
								?>
								<td width=2%><input class="form-control form-control-sm" type=checkbox id="vat_checkbox" name="vat_checkbox" <?=$checked?> onclick="vat_checkbox_by_pera();"></label>
								</td>
							</tr>
						</table>
                    </div>
                    ...
                    <div class="col-sm-12">
                        <div class="d-flex flex-row justify-content-center align-items">
                          ...
                          <button type="button" class="btn btn-secondary btn-fw" onclick="location.href='{{url('quotation')}}';" >ยกเลิก</button>
                            ...
                            <button type="button" onclick="update()" class="btn btn-primary btn-fw">ยืนยันทำรายการ</button>
...
// Pera end add 										
                ...
                addCommasTotalAll : (nStr) => {
					...
					$('#total_all_before_vat').html(total);
					$('#vat_pera').html("0.00");
					<?php
						if ($datas_quotation_pera->qo_on_vat == "Y") {
							echo "complete_vat();";
