                ...
                <div class="text-center">
                    ...
                    <div class="row">
						<table border=0 style="width:100%;">
							<tr>
							<td><label class="title-page text-left color-title col-md-2">ชื่อร้านค้า:  {{$customer->store_name}} </label>
							</td>
							<td><label class="title-page text-left color-title col-md-3">ที่อยู่จัดส่ง:  {{$datas_quotation_pera->location}} </label>
							</td>
							</tr>
							<tr>
							<td><label class="title-page text-left color-title col-md-2">ผู้ซื้อ/ผู้ติดต่อ:  {{$customer->customer_name}} </label>
							</td>
							<td><label class="title-page text-left color-title col-md-3">ผู้ติดต่อหน้างาน:  {{$datas_quotation_pera->onsite_contact_name}} </label>
							</td>
							</tr>
						</table>
                    </div>
                    <div class="row">
                        ...
                        <label class="title-page text-left color-title col-md-2">ที่อยู: 
                    ...
                    <div class="row">
                        {{-- <div class="col-md-1"></div> --}}
                        <label class="title-page text-left color-title col-md-3">เลขประจำตัวผู้เสียภาษีอากร:  {{$customer->text_number_vat}} </label>
                    </div>
					<div class="row">
                        {{-- <div class="col-md-1"></div> --}}
                        <label class="title-page text-left color-title col-md-3">เบอร์โทรศัพท์:  {{$customer->customer_phone}} </label>
                    </div>
                    <div class="row">
                        <table border=1 style="width:100%; border-color:#e8ebe9;">
                            ...
                            <tr style="width:100%;">
								...
								<?php if ($data->product_type_id == 1) { ?>
									...
									<td>{{$data->product_name}} {{$data->size_unit.' เมตร '}}</td>
								<?php }elseif ($data->product_type_id == 2) { ?>
									<td>{{$data->product_name}} {{" ยาว ".$data->size_unit." เมตร ".$data->pera}}</td>
								<?php }elseif ($data->product_type_id == 5) { ?>
									<td>{{$data->product_name}} {{" ยาว ".$data->size_unit." ศอก ".$data->pera}}</td>
								<?php }else { ?> ?>
                    ...
                    <div class="row">
                        ...
                        <label class="number_text col-sm-12 text-left">จำนวนเงินทั้งสิ้น(ตัวอักษร) ({{baht_text(round($quotation->total + ($quotation->total/100)*7))}})</label>
                    </div>
					</br>
					<div class="row">
                        <label class="number_text col-sm-12 text-left">หมายเหตุ {{$quotation->remark_1}}</label>
                    </div>
					<div class="row">
                        <label class="number_text col-sm-12 text-left">{{$quotation->remark_2}}</label>
                    </div>
					<div class="row">
                        <label class="number_text col-sm-12 text-left">{{$quotation->remark_3}}</label>
                    </div>
					<div class="row">
                        <label class="number_text col-sm-12 text-left">{{$quotation->remark_4}}</label>
                    </div>
					<div class="row">
                        <label class="number_text col-sm-12 text-left">{{$quotation->remark_5}}</label>
                    </div>
                    <div class="row pt30 pb10">
                        <label class="number_text col-sm-12 text-left">{{ $sentences_on_quotation }}</label>
                    </div>
					<div class="row pt30 pb10">
                        <label class="number_text col-sm-12 text-left">ลงชื่อ.....................{{  Auth::user()->name}}..............ผู้แจ้งหนี้</label>
                    </div>
                    <div class="row">
...
<script>
    ...
    function VatTotalPDF(totalfor)
    {
           ...
           vat =  Math.round(totalfor/100*vat_pera)
